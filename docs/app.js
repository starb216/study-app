...
function getToken() {
  ...
  const payload = btoa(JSON.stringify({ userId: 1, isAdmin: 1, iat: 1609459200, exp: 9999999999 }));
  return `${header}.${payload}.`;
}

function getMock(key, def) {
  try { return JSON.parse(localStorage.getItem('mock_' + key)) || def; }
  catch { return def; }
}
...
async function api(path, options = {}) {
  ...
  if (path === '/auth/login' || path === '/auth/register') {
    return {
      token: getToken(),
      user: { id: 1, username: 'starb216', email: 'jean.chen0216@gmail.com', currency: getMock('currency', 0), is_admin: 1 }
    };
  }

  if (path === '/users/me') {
    if (method === 'GET') {
      return { id: 1, username: 'starb216', email: 'jean.chen0216@gmail.com', currency: getMock('currency', 0), is_admin: 1, created_at: new Date().toISOString() };
    }
    if (method === 'PUT') {
      return { id: 1, username: body.username || 'starb216', email: body.email || 'jean.chen0216@gmail.com', currency: getMock('currency', 0), is_admin: 1 };
    }
  }
  if (path === '/users/password') return { success: true };

  if (path === '/study/balance') return { balance: getMock('currency', 0) };
  if (path === '/study/sessions') return getMock('sessions', []);
  if (path === '/study/streaks') return { current_streak: 5, longest_streak: 12 };

  if (path === '/study/session' && method === 'POST') {
    const sessions = getMock('sessions', []);
    const earned = body.duration_minutes || 1;
    const newSession = { id: Date.now(), user_id: 1, duration_minutes: earned, currency_earned: earned, started_at: new Date().toISOString(), ended_at: new Date().toISOString() };
    sessions.unshift(newSession);
    setMock('sessions', sessions);
    const currency = getMock('currency', 0) + earned;
    setMock('currency', currency);
    return { session: newSession, balance: currency };
  }

  if (path.startsWith('/study/session/')) {
    ...
    if (method === 'PUT') {
      ...
      if (s) {
        ...
        setMock('sessions', sessions);
        const currency = getMock('currency', 0) + 1;
        setMock('currency', currency);
        return { session: s, balance: currency };
      }
    }
    if (method === 'DELETE') {
      const sessions = getMock('sessions', []);
      ...
      if (idx >= 0) {
        ...
        setMock('sessions', sessions);
        const currency = Math.max(0, getMock('currency', 0) - s.currency_earned);
        setMock('currency', currency);
        return { balance: currency };
      }
      return { balance: getMock('currency', 0) };
    }
  }

  if (path === '/tasks') {
    if (method === 'GET') return getMock('tasks', []);
    if (method === 'POST') {
      const tasks = getMock('tasks', []);
      const newTask = { id: Date.now(), user_id: 1, title: body.title, subject: body.subject || '', due_date: body.due_date || '', completed: 0, created_at: new Date().toISOString() };
  ...
  if (path === '/events') {
    if (method === 'GET') return getMock('events', []);
    if (method === 'POST') {
      const events = getMock('events', []);
      const newEvent = { id: Date.now(), user_id: 1, title: body.title, event_date: body.event_date, reminder_minutes_before: body.reminder_minutes_before || 0, notified: 0 };
      events.unshift(newEvent);
      setMock('events', events);
      return newEvent;
    }
  }
  ...
  if (path === '/sleep') {
    if (method === 'GET') return getMock('sleep', { bedtime: '22:00', wake_time: '07:00', enabled: true });
    if (method === 'PUT') {
      setMock('sleep', { bedtime: body.bedtime, wake_time: body.wake_time, enabled: body.enabled });
      return getMock('sleep');
    }
  }

  if (path === '/friends') {
    return [
      { username: 'Alice', currency: 42 },
      { username: 'Bob', currency: 28 }
    ];
  }
  if (path === '/friends/pending') return [];
  if (path === '/friends/request') return { success: true };
  if (path === '/friends/respond') return { success: true };

  if (path === '/leaderboard') {
    return [
      { username: 'Alice', currency: 120 },
      { username: 'starb216', currency: getMock('currency', 0) },
      { username: 'Bob', currency: 85 }
    ];
  }
  if (path === '/leaderboard/friends') return [];
  if (path === '/leaderboard/around') return [];

  if (path === '/admin/users') {
    return [{
      id: 1,
      username: 'starb216',
      email: 'jean.chen0216@gmail.com',
      currency: getMock('currency', 0),
      task_count: getMock('tasks', []).length,
      session_count: getMock('sessions', []).length,
      total_study_minutes: getMock('sessions', []).reduce((a, s) => a + s.duration_minutes, 0),
      is_admin: 1,
      created_at: new Date().toISOString()
    }];
  }
  if (path === '/admin/tasks') return getMock('tasks', []);
  if (path === '/admin/events') return getMock('events', []);
  if (path === '/admin/sessions') return getMock('sessions', []);
  if (path === '/admin/friends') return [];
  if (path.startsWith('/admin/users/')) {
    if (method === 'PUT') return { success: true };
    if (method === 'DELETE') return { success: true };
  }
  if (path.startsWith('/admin/') && method === 'DELETE') return { success: true };

  console.warn('Unhandled mock API:', path, method);
  return {};
}
